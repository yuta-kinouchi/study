...
class SumoEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.tf = [0,0,0]
        self.feature = [[],[],[],[],[],[],[],[]]
        self.pattern = random.randrange(10)
    ...
    def add_car(self,tf,step):
        ...
        print(self.pattern)
        if self.pattern == 0:
            top    = np.random.poisson(0.4)
            bottom = np.random.poisson(0.4)
            right  = np.random.poisson(0.1)
            left   = np.random.poisson(0.1)
        elif self.pattern == 1:
            top    = np.random.poisson(0.3)
            bottom = np.random.poisson(0.3)
            right  = np.random.poisson(0.2)
            left   = np.random.poisson(0.2)
        elif self.pattern == 2:
            top    = np.random.poisson(0.2)
            bottom = np.random.poisson(0.2)
            right  = np.random.poisson(0.3)
            left   = np.random.poisson(0.3)
        elif self.pattern == 3:
            top    = np.random.poisson(0.1)
            bottom = np.random.poisson(0.1)
            right  = np.random.poisson(0.4)
            left   = np.random.poisson(0.4)
        elif self.pattern == 4:
            top    = np.random.poisson(0.1)
            bottom = np.random.poisson(0.1)
            right  = np.random.poisson(0.1)
        ...
        for i in range(top):
            if np.random.uniform(0,1) > 0.1:
                traci.vehicle.addFull(vehID=x +str(i)  + "tb",routeID="t_b", typeID='DEFAULT_VEHTYPE')
            else:
                traci.vehicle.addFull(vehID=x +str(i) + "tl",routeID="t_r", typeID='DEFAULT_VEHTYPE')

        for i in range(bottom):
            if np.random.uniform(0,1) > 0.1:
                traci.vehicle.addFull(vehID=x+str(i) + "bt",routeID="b_t", typeID='DEFAULT_VEHTYPE')
            else:
                traci.vehicle.addFull(vehID=x+str(i) + "br",routeID="b_l", typeID='DEFAULT_VEHTYPE')
        
        for i in range(right):
            if np.random.uniform(0,1) > 0.1:
                traci.vehicle.addFull(vehID=x+str(i) + "rl",routeID="r_l", typeID='DEFAULT_VEHTYPE')  
            else:
                traci.vehicle.addFull(vehID=x+str(i) + "rt",routeID="r_b", typeID='DEFAULT_VEHTYPE')  

        for i in range(left):
            if np.random.uniform(0,1)  > 0.1:
                traci.vehicle.addFull(vehID=x+str(i) + "lr",routeID="l_r", typeID='DEFAULT_VEHTYPE')   
            else:
                traci.vehicle.addFull(vehID=x+str(i) + "lb",routeID="l_t", typeID='DEFAULT_VEHTYPE')  



        # k = np.random.poisson(0.3)
        # if tf[0] == 0:
        #     p = 0.125
        # elif tf[0] == 1:
        #     p = 0.25
